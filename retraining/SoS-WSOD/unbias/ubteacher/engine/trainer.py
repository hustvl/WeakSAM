...
class UBTeacherTrainer(DefaultTrainer):
    def __init__(self, cfg):
        ...
        # PS: added to filter with multi-label, default: False
        self.has_multi_label = cfg.SEMISUPNET.HAS_MULTI_LABEL
        ...
        if comm.get_world_size() > 1:
            ...
            if cfg.MODEL.RESNETS.NORM == "SyncBN":
    ...
    # ================== Pseduo-labeling ==================
    # =====================================================
    def threshold_bbox(self, unlabe_data_inst, proposal_bbox_inst, thres=0.7, proposal_type="roih"):
        if proposal_type == "rpn":
            ...
            if self.has_multi_label:
                gt_multi_label = unlabe_data_inst["multi_label"]
                pred_classes = proposal_bbox_inst.pred_classes
                for i in range(len(pred_classes)):
                    if pred_classes[i] not in gt_multi_label:
                        valid_map[i] = False
            # create instances containing boxes and gt_classes
            image_shape = proposal_bbox_inst.image_size
            new_proposal_inst = Instances(image_shape)
    ...
    def process_pseudo_label(
        ...
        for unlabe_data_inst, proposal_bbox_inst in zip(unlabel_data_k, proposals_rpn_unsup_k):
            # thresholding
            if psedo_label_method == "thresholding":
                proposal_bbox_inst = self.threshold_bbox(
                    unlabe_data_inst, proposal_bbox_inst, thres=cur_threshold, proposal_type=proposal_type
                )
            else:
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            if self.cfg.SEMISUPNET.BURN_UP_WITH_STRONG_AUG:
                label_data_q.extend(label_data_k)
                record_dict, _, _, _ = self.model(label_data_q, branch="supervised")
            else:
                record_dict, _, _, _ = self.model(label_data_k, branch="supervised")
            # weight losses
            loss_dict = {}
            for key in record_dict.keys():
                if key[:4] == "loss":
            ...
            if self.iter == self.cfg.SEMISUPNET.BURN_UP_STEP and self.cfg.SEMISUPNET.BURN_UP_STEP > 0:
            ...
            (
                ...
                unlabel_data_k, proposals_rpn_unsup_k, cur_threshold, "rpn", "thresholding"
            )
            joint_proposal_dict["proposals_pseudo_rpn"] = pesudo_proposals_rpn_unsup_k
            # Pseudo_labeling for ROI head (bbox location/objectness)
            pesudo_proposals_roih_unsup_k, _ = self.process_pseudo_label(
                unlabel_data_k, proposals_roih_unsup_k, cur_threshold, "roih", "thresholding"
            )
