        ...
        if len(all_predictions) == 1:
            ...
            for p in all_predictions:
                for key in list(p.keys()):
                    if key not in all_prediction:
                        all_prediction[key] = p[key]
                    else:
...
#---------------------------------------- Edited by Colez. ----------------------------------------#
        result_save_path_template = "/home/junweizhou/WeakSAM/SoS-WSOD/pseudo_truetrain12_lr0015_mist_without_drop/results/VOC2012/Main/comp3_det_test_{}.txt"
        ...
        with tempfile.TemporaryDirectory(prefix="pascal_voc_eval_") as dirname:
            ...
            for cls_id, cls_name in enumerate(self._class_names):
                ...
                if os.path.isdir("/home/junweizhou/WeakSAM/SoS-WSOD/pseudo_truetrain12_lr0015_mist_without_drop/results/") and "voc_2012_test" in self._dataset_name:
                    print("saving result to {}".format(result_save_path_template.format(cls_name)))
                    result_save_file = open(result_save_path_template.format(cls_name), "w")
                    result_save_file.write("\n".join(lines)+"\n")
                    
#---------------------------------------- Edited by Colez. ----------------------------------------#

                if "voc_2012_test" in self._dataset_name:
                    continue
