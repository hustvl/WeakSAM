...
# Here for the result showing. Changed for alignment of results.
    def show_result(self,
        ...
        draw_boxes(img, bboxes, labels, out_file)

        if not (show or out_file):
            return img


def draw_boxes(image, boxes, labels, out_file):
    img_store = overlay_boxes(image, boxes, labels)
    if out_file is not None:
        cv2.imwrite(out_file, img_store)
    return img_store
